...
-- employees 테이블의 부서 id와 일치하는 deparmtents 테이블의 부서 id를
...
테이블 이름이 너무 길 시에는 ALIAS(AS)를 작성하여 칭합니다.
...
2. location_id 값과 같은 값을 가지는 데이터를 department_id에서 찾아서 조인
3. 위의 조인한 결과와 동일한 department_id를 가진 employees 테이블의 데이터를 찾아 조인
4. 위에 결과의 jobs 테이블을 비교하여 조인하고 최종 결과를 출력
*/
...
/*
employees 테이블에 존재하고 departments 테이블에는 존재하지 않아도
(+)가 붙지 않은 테이블을 기준으로 하여 departments 테이블이 조인에
참여하라는 의미를 부여하기 위한 기호를 붙입니다.
외부조인을 사용했더라고, 이후에 내부 조인을 사용하면
내부조인을 무조건적으로 인식합니다.
*/
