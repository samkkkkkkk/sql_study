...
SELECT
    *
FROM info i LEFT JOIN auth a -- OUTER는 생략 가능하지만 RIGT나 LEFT는 명시해줘야 한다.
ON i.auth_id = a.auth_id;
...
-- 좌측 테이블과 우측 테이블 데이터를 모두 얽어 중복된 데이터는 삭제되는 외부조인
SELECT 
    *
...
-- 동일 테이블 컬러을 통해 기존에 존재하는 값을 매칭시켜 가져올 때 사용합니다.
SELECT 
