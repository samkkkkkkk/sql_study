...
IS --선언부
    v_msg VARCHAR(30) := 'Hello Procedual!';
BEGIN --실행부
...
EXEC my_new_job_proc('JOB2', 'test job1', 10000, 20000);
...
BEGIN
    ...
    -- 이미 존재한더면 1, 존재하지 않는다면 0 -> v_cnt에 저장.
    SELECT 
        COUNT(*)
    INTO 
        v_cnt
...
BEGIN
    ...
    my_new_job_proc('CEO2', 'test_ceo', 20000, 80000, msg);
    dbms_output.put_line(msg);
END;
...
BEGIN
    ...
    IF v_cnt = 0 THEN 
        dbms_output.put_line(p_job_id || '는 테이블에 조재하지 않습니다.');
        RETURN; -- 프로시저 강제종료
    END IF;
